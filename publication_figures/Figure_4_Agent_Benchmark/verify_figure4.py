...
def main() -> None:
    ...
    required = [
        ROOT / "Figure_4_Agent_Benchmark.svg",
        ROOT / "Figure_4_Agent_Benchmark.pdf",
        ROOT / "Figure_4_Agent_Benchmark.tiff",
        ROOT / "Figure_4_Agent_Benchmark.png",
    ...
    svg_text = (ROOT / "Figure_4_Agent_Benchmark.svg").read_text(
        encoding="utf-8"
    )
    ...
    with Image.open(ROOT / "Figure_4_Agent_Benchmark.tiff") as image:
        checks["tiff_minimum_dimensions"] = (
            image.width >= 4000 and image.height >= 3500
        )
        dpi = image.info.get("dpi", (0, 0))
        checks["tiff_600dpi"] = round(float(dpi[0])) == 600
    with Image.open(ROOT / "Figure_4_Agent_Benchmark.png") as image:
        dpi = image.info.get("dpi", (0, 0))
    ...
    gate = {
        "status": "FIGURE4_QA_PASSED" if not failed else "FIGURE4_QA_FAILED",
        "success": not failed,
    ...
    (ROOT / "FIGURE4_QA_GATE.json").write_text(
