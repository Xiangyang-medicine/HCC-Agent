...
def draw(ax: plt.Axes, letter: str = "d") -> None:
    ...
    panel_heading(ax, letter, "Outcome-blind standardized space",
                  "Within-cohort gene-wise z-scores; no survival outcomes used")
    ax.set_xlabel(f"PC1 ({v1:.1f}% variance)")
    ax.set_ylabel(f"PC2 ({v2:.1f}% variance)")
    quiet_axis(ax, "both")
    ax.legend(frameon=False, loc="best", handletextpad=0.35)
    ax.text(0.02, 0.02, "Input QC only; not evidence of biological equivalence",
...
def main() -> None:
    ...
    draw(ax)
    save_publication_figure(fig, HERE / "panel_d_standardized_expression_space")
    plt.close(fig)
